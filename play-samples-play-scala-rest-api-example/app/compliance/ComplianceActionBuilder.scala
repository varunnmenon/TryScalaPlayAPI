...
class ComplianceActionBuilder  @Inject()(messagesApi: MessagesApi,
  ...
  type ComplianceRequestBlock[A] = ComplianceRequest[A] => Future[Result]

  private val logger = Logger(this.getClass)

  override def invokeBlock[A](request: Request[A],
                              block: ComplianceRequestBlock[A]): Future[Result] = {
...
case class ComplianceControllerComponents @Inject()(
                                                     complianceActionBuilder: ComplianceActionBuilder,
...
/**
  ...
  */
class ComplianceBaseController @Inject()(pcc: ComplianceControllerComponents)
  extends BaseController
    with RequestMarkerContext {
  override protected def controllerComponents: ControllerComponents = pcc

  def PostAction: ComplianceActionBuilder = pcc.complianceActionBuilder

  def postResourceHandler: ComplianceResourceHandler = pcc.complianceResourceHandler
}
